...
def run(dataset, config):
    ...
    if sort_metric is None:
        ...
        log.warning(
            "Performance metric %s not supported, defaulting to AUTO.", config.metric
        )
    ...
    try:
        ...
        jvm_memory = (
            str(round(config.max_mem_size_mb * 2 / 3)) + "M"
        )  # leaving 1/3rd of available memory for XGBoost
        ...
        train = None
        if (
            version.parse(h2o.__version__) >= version.parse("3.32.1")
        ...
        monitor = (
            BackendMemoryMonitoring(
                interval_seconds=config.ext.monitoring.interval_seconds,
                check_on_exit=True,
        ...
        def infer(path: str):
            ...
            batch = h2o.import_file(
                path,
                col_names=train.names,
                destination_frame=frame_name(filename, config),
                **import_kwargs,
            )
    ...
    try:
        ...
        for m in models:
            ...
            all_models_se = next(
                (
                    ...
                    if mid.startswith("StackedEnsemble_AllModels")
                ),
                None,
            )
        ...
        if "model_predictions" in artifacts:
            ...
            for mid in lb["model_id"]:
                ...
                write_preds(
                    preds, os.path.join(predictions_dir, mid, "predictions.csv")
                )
        ...
        if "logs" in artifacts:
            ...
            write_csv(
                automl.event_log.as_data_frame(),
                os.path.join(logs_dir, "aml_event_log.csv"),
            )
    except Exception:
...
def extract_preds(
    ...
    h2o_truth = test[:, dataset.target.index].as_data_frame(
