...
def run(dataset, config):
    ...
    loss_fn, continuous_loss_fn = (
        metrics_to_loss_mapping[config.metric]
        if config.metric in metrics_to_loss_mapping
        else (None, False)
    )
    if loss_fn is None:
        log.warning(
            "Performance metric %s not supported: defaulting to %s.",
            config.metric,
    ...
    if "algo" in training_params:
        training_params["algo"] = eval(
            training_params["algo"]
        )  # evil eval: use get_extensions instead once https://github.com/openml/automlbenchmark/pull/141 is merged
