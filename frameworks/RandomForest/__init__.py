...
def run(dataset: Dataset, config: TaskConfig):
    ...
    X_train, X_test = (
        impute_array(dataset.train.X_enc, dataset.test.X_enc)
        if encode
        else (dataset.train.X, dataset.test.X)
    )
    y_train, y_test = (
        (dataset.train.y_enc, dataset.test.y_enc)
        if encode
        else (dataset.train.y, dataset.test.y)
    )
