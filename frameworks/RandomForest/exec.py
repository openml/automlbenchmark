...
def run(dataset, config):
    ...
    with Timer() as training:
        ...
        while True:
            ...
            log.info(
                f"Model trained {len(rf.estimators_):6d} trees in {int(training_times[-1]):6d} seconds using {int(memory_usage[-1]):6d}mb memory."
            )

            will_run_out_of_memory = (
                extrapolate_with_worst_case(memory_usage)
                >= config.max_mem_size_mb * memory_margin
            )
            will_run_out_of_time = (
                extrapolate_with_worst_case(training_times)
                >= config.max_runtime_seconds * time_margin
            )
