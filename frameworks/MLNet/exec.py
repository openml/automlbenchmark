...
def run(dataset: Dataset, config: TaskConfig):
    ...
    MLDOTNET_CLI_TELEMETRY_OPTOUT = config.framework_params.get(
    ...
    output_dir = (
        output_subdir("models", config=config)
        if "models" in artifacts
        else tmp_output_folder
    )
    log_dir = (
        output_subdir("logs", config=config)
        ...
        with open(train_result_json, "r") as f:
            ...
            if config.type == "classification":
                prediction_df = pd.read_csv(
                    output_prediction_path, dtype={"PredictedLabel": "object"}
                )
