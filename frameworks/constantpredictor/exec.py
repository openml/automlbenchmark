...
def run(dataset: Dataset, config: TaskConfig):
    ...
    X_train = unsparsify(
        dataset.train.X_enc if encode else dataset.train.X, fmt="array"
    )
    y_train = unsparsify(
        dataset.train.y_enc if encode else dataset.train.y, fmt="array"
    )
    ...
    if config.measure_inference_time:
        inference_times["file"] = measure_inference_times(
            infer, dataset.inference_subsample_files(fmt="parquet")
        )
