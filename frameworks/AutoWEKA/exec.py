...
def run(dataset: Dataset, config: TaskConfig):
    ...
    metrics_mapping = dict(acc="errorRate", auc="areaUnderROC", logloss="kBInformation")
    metric = (
    ...
    if dataset.target.index != len(dataset.predictors):
        train_file = reorder_dataset(
            dataset.train.path, target_src=dataset.target.index
        )
    ...
    if memLimit == "auto":
        memLimit = max(
            min(
                config.max_mem_size_mb, math.ceil(config.max_mem_size_mb / parallelRuns)
            ),
            1024,
        )  # AutoWEKA default memLimit
