...
def extrapolate_with_worst_case(values: List[float], n: int = 5) -> float:
    ...
    n = min(len(values), n)
    return values[-1] + max(
        v_next - v_prev for v_prev, v_next in zip(values[-n:], values[-n + 1 :])
    )
...
def run(dataset, config):
    ...
    below_default = pick_values_uniform(start=1, end=default_value, length=5 + 1)[
        :-1
    ]  # 5 below
    above_default = pick_values_uniform(
        start=default_value, end=n_features, length=10 + 1 - len(below_default)
    )[1:]  # 5 above
    ...
    with Timer() as training:
        while max_features_values:
            ...
            if time_per_value < last_initial_fit_time:
                log.info(
                    "Expect to exceed time constraints on next first fit, "
            ...
            while True:
                try:
                    ...
                    max_feature_scores[value].append(
                        statistics.mean(cv_result["test_score"])
                    )
                ...
                if random_forests[0].n_estimators >= final_forest_size:
                    log.info(
                        "Stop training because desired forest size has been reached."
                    )
                    break
                if (
                    extrapolate_with_worst_case(training_times) - training_times[0]
                    >= time_per_value
                ):
                    log.info(
                        f"Stop training after fitting {random_forests[0].n_estimators} trees because it expects to exceed its time budget."
                    )
                    break
                elif (
                    extrapolate_with_worst_case(memory_usage)
                    >= config.max_mem_size_mb * memory_margin
                ):
                    log.info(
                        f"Stop training after fitting {random_forests[0].n_estimators} trees because it expects to exceed its memory budget."
                    )
                    break
        ...
        for max_feature, scores in max_feature_scores.items():
            ...
            stats_for_max_feature = zip(
                scores,
                tuning_durations[max_feature][1:],
                memory_usage_by[max_feature][1:],
            )
            for i, (score, duration, memory) in enumerate(
                stats_for_max_feature, start=1
            ):
                summary.append(
                    {
                        ...
                        "duration delta (s)": duration
                        - tuning_durations[max_feature][i - 1],
                        "memory delta (mb)": memory
                        - memory_usage_by[max_feature][i - 1],
                    }
                )
        summary = pd.DataFrame.from_records(summary)
        log.debug("Report of tuning iterations:\n %s \n" % summary.to_string())
        log.info(
            "Report final tunings:\n %s \n"
            % summary.groupby("max_features")
            .last()
            .sort_values(by="duration total (s)")
            .to_string()
        )

        # Iteratively fit a random forest with our "optimal" `max_features`
        _, best_value = max(
            (scores[-1], value) for value, scores in max_feature_scores.items()
        )
        ...
        while True:
            ...
            if rf.n_estimators == final_forest_size:
                ...
                break
            if (
                extrapolate_with_worst_case(training_times)
