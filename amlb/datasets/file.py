...
class FileLoader:
    def __init__(self, cache_dir=None):
        self._cache_dir = (
            cache_dir if cache_dir else tempfile.mkdtemp(prefix="amlb_cache")
        )
    ...
    def load(self, dataset, fold=0):
        ...
        paths = self._extract_train_test_paths(
            dataset.path if "path" in dataset else dataset,
            fold=fold,
        ...
        if ext == ".arff":
            return ArffDataset(
                train_path, test_path, target=target, features=features, type=type_
            )
        elif ext == ".csv":
            return CsvDataset(
                train_path, test_path, target=target, features=features, type=type_
    ...
    def _extract_train_test_paths(self, dataset, fold=None, name=None):
        if isinstance(dataset, (tuple, list)):
            ...
            return dict(
                train=[
                    self._extract_train_test_paths(p, name=name)["train"][0]
                    if i == fold
                    else None
        ...
        if os.path.exists(dataset):
            if os.path.isfile(dataset):
                if is_archive(dataset):
                    ...
                    if not os.path.exists(
                        dest_folder
                    ):  # don't uncompress if previously done
                ...
                train_matches = [
                    m for m in [train_search_pat.search(f) for f in files] if m
                ]
                test_matches = [
                ...
                while fold >= 0:
                    train = next(
                        (m[0] for m in train_matches if m[2] == str(fold)), None
                    )
            ...
            if name is None:
                ...
                cached_file = os.path.join(
                    self._cache_dir, name, os.path.basename(dataset)
                )
...
class FileDataset(Dataset):
    def __init__(
        ...
        features: list[ns | str] | None = None,
        type: str | None = None,
    ):
        """
    ...
    def type(self) -> DatasetType:
        ...
        return (
            DatasetType[self._type]
            if self._type is not None
            else DatasetType.regression
            if self.target.values is None
            else DatasetType.binary
            if len(self.target.values) == 2
...
class FileDatasplit(Datasplit):
    ...
    def data_path(self, format):
        ...
        if format not in supported_formats:
            ...
            raise ValueError(
                f"Dataset {name} is only available in one of {supported_formats} formats."
            )
    ...
    def _find_target_feature(self, features: List[Feature]):
        ...
        default_target = next(
            (f for f in features if f.name.lower() in ["target", "class"]), features[-1]
        )
        return (
            features[target]
            if isinstance(target, int)
            else next(f for f in features if f.name == target)
    ...
    def _set_feature_as_target(self, target: Feature):
        ...
        if ds_type and DatasetType[ds_type] in [
            DatasetType.binary,
            DatasetType.multiclass,
        ]:
            if not target.is_categorical():
                log.warning(
                    "Forcing target column `%s` as 'category' for classification problems: was originally detected as '%s'.",
                    target.name,
                    target.data_type,
                )
...
class TimeSeriesDataset(FileDataset):
    def __init__(self, path, fold, target, features, cache_dir, config):
        super().__init__(
            None, None, target=target, features=features, type="timeseries"
        )
        if config["forecast_horizon_in_steps"] is None:
            raise AssertionError(
                "Task definition for timeseries must include `forecast_horizon_in_steps`"
            )
        if config["freq"] is None:
            raise AssertionError("Task definition for timeseries must include `freq`")
        if config["seasonality"] is None:
            raise AssertionError(
                "Task definition for timeseries must include `seasonality`"
            )

        full_data = read_csv(path)
        if config["id_column"] is None:
            log.warning(
                "Warning: For timeseries task, setting undefined `id_column` to `item_id`"
            )
        ...
        if config["timestamp_column"] is None:
            log.warning(
                "Warning: For timeseries task, setting undefined `timestamp_column` to `timestamp`"
            )
        ...
        full_data[self.timestamp_column] = pd.to_datetime(
            full_data[self.timestamp_column]
        )
        ...
        train_path, test_path = self.save_train_and_test_splits(
            full_data, fold=fold, save_dir=save_dir
        )

        self._train = CsvDatasplit(
            self, train_path, timestamp_column=self.timestamp_column
        )
        self._test = CsvDatasplit(
            self, test_path, timestamp_column=self.timestamp_column
        )
        ...
        self.repeated_item_id = (
            self.test.data[self.id_column].astype("category").cat.codes.to_numpy()
        )
    ...
    def save_train_and_test_splits(self, full_data, fold, save_dir):
        ...
        if fold > 0:
            ...
            full_data = full_data.groupby(self.id_column, as_index=False).nth(
                slice(None, -steps_to_remove)
            )
        train_data = full_data.groupby(self.id_column, as_index=False).nth(
            slice(None, -self.forecast_horizon_in_steps)
        )
        test_data = full_data.groupby(self.id_column, as_index=False).nth(
            slice(-self.forecast_horizon_in_steps, None)
        )
    ...
    def compute_seasonal_error(self):
        ...
        seasonal_diffs = (
            train_data_with_index[self.target.name]
            .groupby(level=self.id_column)
            .diff(self.seasonality)
            .abs()
        )
        abs_seasonal_error = (
            seasonal_diffs.groupby(level=self.id_column).mean().fillna(1.0).values
        )
        ...
        if self._ds is None:
            if self.dataset._dtypes is None:
                ...
                dt_conversions = {
                    ...
                    or (
                        name == self.dataset._target
                        and self.dataset._type is not None
                        and DatasetType[self.dataset._type]
                        in [DatasetType.binary, DatasetType.multiclass]
                    )
                }
                ...
                self._ds = read_csv(
                    self.path,
                    dtype=self.dataset._dtypes.to_dict(),
        ...
        attributes = [
            (
                ...
                (
                    ...
                    else [str(cat) for cat in sorted(dt.categories.values)]
                    if pat.is_categorical_dtype(dt)
                    else "STRING"
                ),
            )
