...
def is_sparse(data):
    np, pd, sp = _import_data_libraries()
    return (
        (sp and isinstance(data, sp.spmatrix))  # sparse matrix
        or (
            pd and isinstance(data, pd.Series) and pd.api.types.is_sparse(data.dtype)
        )  # sparse Series
        or (
            pd
            and isinstance(
                data, pd.DataFrame
            )  # if one column is sparse, the dataframe is considered as sparse
...
def _unsparsify(data, fmt=None):
    ...
    if sp and isinstance(data, sp.spmatrix):
        ...
        return (
            ...
            if fmt == "array"
            else _pd_to_dense(pd, data)
            if fmt == "dense" and is_sparse(data)
            else data
        )
    else:
        return data
...
def _pd_to_dense(pd, df):
    ...
    data = {
        k: (v.sparse.to_dense() if hasattr(v, "sparse") else v) for k, v in df.items()
    }
...
def deserialize_data(path, config: Optional[ns] = None):
    ...
    if ext == ".npy":
        ...
        if ser == "pickle":
            ...
            _, ext2 = os.path.splitext(base)
            if (
                config.sparse_dataframe_deserialized_format is None
                and ext2 == ".sparse"
            ):
        ...
        raise SerializationError(
            f"Can not deserialize file `{path}` in unknown format."
        )
