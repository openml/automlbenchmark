...
class AWSBenchmark(Benchmark):
    ...
    def fetch_results(cls, instances_file, instance_selector=None):
        ...
        instance_selector = (
            (lambda *_: True) if instance_selector is None else instance_selector
        )
    ...
    def reconnect(cls, instances_file):
        ...
        def to_job(iid, inst):
            ...
            job = Job(
                inst.key, raise_on_failure=rconfig().job_scheduler.exit_on_job_failure
            )
            job.instance_id = iid
    ...
    def __init__(
        ...
        self.region = (
            region
            if region
            else rconfig().aws.region
            if rconfig().aws["region"]
    ...
    def setup(self, mode):
        ...
        self.uploaded_resources = (
            self._upload_resources() if mode != SetupMode.script else []
        )
        ...
        if self.parallel_jobs > 1:
            if rconfig().aws.minimize_instances:
                ...
                try:
                    jobs = flatten(
                        [
                            self._make_aws_job([task_def.name], folds)
            ...
            try:
                task_names = (
                    None if tasks is None else [task_def.name for task_def in task_defs]
                )
    ...
    def _create_job_runner(self, jobs):
        if self.parallel_jobs == 1:
            ...
            queueing_strategy = (
                MultiThreadingJobRunner.QueueingStrategy.enforce_job_priority
            )
    ...
    def _exec_stop(self):
        ...
        try:
            ...
            log.warning("Error while shutting down thread pool executor: %s", e)
        finally:
            self.exec = None
    ...
    def _exec_send(self, fn, *args, **kwargs):
        if self.exec is not None:
            ...
            try:
                ...
                logging.warning(f"Failed to execute function {fn.__name__}: %s", e)
    ...
    def _job_reschedule(self, job, reason=None, fallback=None):
        ...
        if not job.ext.retry:
            ...
            job.ext.retry = retry_after(
                start_delay, delay_fn, max_retries=js.max_attempts - 1
            )
        ...
        if wait is None:
            if fallback and fallback(job, reason):
                ...
                log.error(
                    "Aborting job %s after %s attempts: %s.",
                    job.name,
                    js.max_attempts,
                    reason,
                )
    ...
    def _make_aws_job(self, task_names=None, folds=None):
        ...
        task_def = (
            self._get_task_def(task_names[0])
            if len(task_names) >= 1
            else self._get_task_def(
                "__defaults__", include_disabled=True, fail_on_missing=False
            )
            or ns(name="all")
        )
        ...
        instance_def = ns()
        instance_def.type = (
            task_def.ec2_instance_type
            if "ec2_instance_type" in task_def
            else ".".join(
                [
        ...
        instance_def.volume_type = (
            task_def.ec2_volume_type
            if "ec2_volume_type" in task_def
            else rconfig().aws.ec2.volume_type
        )
        instance_def.volume_size = (
            math.ceil(
                (task_def.min_vol_size_mb + rconfig().benchmarks.os_vol_size_mb)
                / 1024.0
            )
        ...
        timeout_secs = (
            task_def.max_runtime_seconds
            if "max_runtime_seconds" in task_def
        ...
        def _setup(_self):
            ...
            if _self.ext.instance_type is None and spot_config.enabled:
                _self.ext.instance_type = (
                    InstanceType.Spot_Block
                    if spot_config.block_enabled
                    else InstanceType.Spot
                )
            if _self.ext.wait_min_secs:
                _self.ext.interrupt = interrupt = threading.Event()
                countdown(
                    _self.ext.wait_min_secs,
        ...
        def _run(_self):
            try:
                resources_root = (
                    "/custom" if rconfig().aws.use_docker else "/s3bucket/user"
                )
                ...
                try:
                    if isinstance(e, AWSError) and e.retry:
                        log.info(
                            "Job %s couldn't start (%s), rescheduling it.",
                            _self.name,
                            e,
                        )
                        self._job_reschedule(
                            _self, reason=str(e), fallback=self._spot_fallback
                        )
                        return
                ...
                self._exec_send(
                    (lambda reason, **kwargs: self._save_failures(reason, **kwargs)),
                    e,
        ...
        def _on_state(_self, state):
            if state == JobState.completing:
                ...
                log_time = datetime.datetime.now(datetime.timezone.utc).strftime(
                    "%Y-%m-%dT%H:%M:%S"
                )
                if failure:
                    self._exec_send(
                        (
                            lambda reason, **kwargs: self._save_failures(
                                reason, **kwargs
                            )
                        ),
                        failure,
                ...
                self._stop_instance(
                    _self.ext.instance_id, terminate=_self.ext.terminate, wait=False
                )
                ...
                self._stop_instance(
                    _self.ext.instance_id, terminate=_self.ext.terminate
                )
                try:
    ...
    def _wait_for_results(self, job):
        ...
        def log_console():
            ...
            try:
                ...
                if "Output" in output:
                    output = output[
                        "Output"
                    ]  # note that console_output only returns the last 64kB of console
                    new_lines, last_line = tail(
                        output,
                    ...
                    if new_lines:
                        new_log = "\n".join(
                            [f"[{job.ext.instance_id}]>{line}" for line in new_lines]
                        )
        ...
        while not interrupt.is_set():
            inst_desc = (
                self.instances[job.ext.instance_id]
                if job.ext.instance_id in self.instances
                else ns()
            )
            if inst_desc["abort"]:
            ...
            try:
                ...
                log.info(
                    "[%s] checking job %s on instance %s: %s [%s].",
                    datetime_iso(),
                    job.name,
                    job.ext.instance_id,
                    state,
                ...
                if state_code == 16:
                    ...
                    try:
                        if any(
                            state in state_reason_msg
                            for state in rconfig().aws.job_scheduler.retry_on_states
                        ):
                            log.warning(
                                "Job %s was aborted due to '%s', rescheduling it.",
                                job.name,
                                state_reason_msg,
                            )
                            self._job_reschedule(
                                job,
                                reason=state_reason_msg,
                                fallback=self._spot_fallback,
                            )
                    finally:
                        interrupt.set()
            except JobError as je:
    ...
    def _get_cpu_activity(self, iid, delta_minutes=60, period_minutes=5):
        ...
        return [
            ...
            for activity in sorted(
                resp["Datapoints"], key=op.itemgetter("Timestamp"), reverse=True
            )
        ]
    ...
    def _monitoring_stop(self):
        ...
        try:
            ...
            log.warning("Exception while stopping monitoring %s", e)
        finally:
            self.monitoring = None
        ...
        try:
            if ec2_config.subnet_id:
                ...
                if subnet.available_ip_address_count == 0:
                    log.warning(
                        "No IP available on subnet %s, parallelism (%s) may be too high for this subnet.",
                        subnet.id,
                        self.parallel_jobs,
                    )
            ...
            volume_tags = (ec2_config.volume_tags or instance_tags) | ns(
                Name=f"amlb_{inst_key}"
            )
            instance_params = dict(
                BlockDeviceMappings=[
                    dict(DeviceName=ec2_config.root_device_name, Ebs=ebs)
                ],
                ...
                UserData=self._ec2_startup_script(
                    inst_key, script_params=script_params, timeout_secs=timeout_secs
                ),
            )
            if ec2_config.availability_zone:
                instance_params.update(
                    Placement=dict(AvailabilityZone=ec2_config.availability_zone)
                )
            ...
            if instance_type in [InstanceType.Spot, InstanceType.Spot_Block]:
                ...
                if instance_type is InstanceType.Spot_Block:
                    duration_min = (
                        math.ceil(timeout_secs / 3600) * 60
                    )  # duration_min must be a multiple of 60
    ...
    def _stop_instance(self, instance_id, terminate=None, wait=True):
        ...
        try:
            log.info(
                "%s EC2 instances %s.",
                "Terminating" if terminate else "Stopping",
                instance_id,
            )
            wait_config = rconfig().aws.ec2.terminate_waiter
            wait = wait and wait_config is not None and wait_config.max_attempts > 0
            waiter = (
                self.ec2.meta.client.get_waiter(
                    "instance_terminated" if terminate else "instance_stopped"
                )
                if wait
                else None
            )
            ...
            log.info(
                "%s EC2 instances %s with response %s.",
    ...
    def _save_failures(self, reason, **kwargs):
        try:
            ...
            write_csv(
                [
                    (
                        self._forward_params["framework_name"],
                        self._forward_params["benchmark_name"],
    ...
    def _s3_key(
        self, main_dir, *subdirs, instance_key_or_id=None, absolute=False, encode=False
    ):
    ...
    def _create_s3_bucket(self, bucket_name=None, auto_create=True):
        ...
        try:
            ...
            if error_code == 404 and auto_create:
                log.info(
                    "%s bucket doesn't exist, creating it in region %s.",
                    bucket_name,
                    self.region,
                )
                ...
                if error_code == 403:
                    log.error(
                        ...
                        " (cf. docs/README.md for more details).",
                        bucket_name,
                    )
                elif error_code == 404:
                    log.error(
    ...
    def _delete_s3_bucket(self):
        if self.bucket:
            ...
            if len(to_delete) > 0:
                log.info(
                    "Deleting objects from S3 bucket %s: %s",
                    self.bucket.name,
                    to_delete,
                )
                self.bucket.delete_objects(Delete=dict(Objects=to_delete, Quiet=True))
    ...
    def _rel_path(self, res_path):
        ...
        return (
            os.path.relpath(res_path, start=rconfig().input_dir)
            if in_input_dir
            else os.path.relpath(res_path, start=rconfig().user_dir)
            if in_user_dir
    ...
    def _upload_resources(self):
        ...
        upload_files = list_all_files(
            upload_paths, file_filter(exclude=rconfig().aws.resource_ignore)
        )
        ...
        for res in upload_files:
            ...
            if upload_path is None:
                ...
                continue
            log.info(
                "Uploading `%s` to `%s` on s3 bucket %s.",
                res,
                upload_path,
                self.bucket.name,
            )
    ...
    def _delete_resources(self):
        if self.uploaded_resources is None:
            return
        log.info(
            "Deleting uploaded resources `%s` from s3 bucket %s.",
            self.uploaded_resources,
            self.bucket.name,
        )
        self.bucket.delete_objects(
            Delete=dict(Objects=[dict(Key=res) for res in self.uploaded_resources])
        )
    ...
    def _download_results(self, instance_id):
        ...
        def download_file(obj, dest, dest_display_path=None):
            ...
            try:
                log.info(
                    "Downloading `%s` from s3 bucket %s to `%s`.",
                    obj.key,
                    self.bucket.name,
                    dest_display_path,
                )
                ...
                log.exception(
                    "Failed downloading `%s` from s3 bucket %s: %s",
                    obj.key,
                    self.bucket.name,
                    str(e),
                )
            ...
            for obj in objs:
                ...
                try:
                    ...
                    if is_result and not success:
                        self._exec_send(
                            lambda path: self._save_global(Scoreboard.from_file(path)),
                            dest_path,
                        )
                        success = True
                except Exception as e:
        ...
        if not success and error is None:
            if len(objs) > 0:
                error = NoResultError(
                    f"No {Scoreboard.results_file} file found among the result artifacts: "
                    f"check the remote logs if available or the local logs to understand what happened on the instance."
                )
            else:
                error = NoResultError(
                    "No result artifacts, either the benchmark failed to start, or the instance got killed: "
                    "check the local logs to understand what happened on the instance."
                )
    ...
    def _results_summary(self, scoreboard=None):
        log.info("Result summary not available for AWS mode (reference files instead).")
    ...
    def _create_instance_profile(self):
        ...
        bucket_prefix = (
            (s3c.bucket + "-")
            if (s3c.temporary and not iamc.temporary)
            else self.bucket.name
        )
        ...
        if not irole:
            ec2_role_trust_policy_json = json.dumps(
                {  # trust role
        ...
        try:
            ...
            log.info(
                "Instance profile %s doesn't exist, creating it: [%s].",
                profile_name,
                str(e),
            )
        if not iprofile:
    ...
    def _delete_iam_entities_from_role(self, role_name):
        ...
        try:
            ...
            for policy in irole.policies.all():
                log.info(
                    "Deleting role policy %s from role %s.",
                    policy.name,
                    policy.role_name,
                )
                policy.delete()
            ...
            log.info(
                "Role %s doesn't exist, skipping its deletion: [%s]",
                iamc.role_name,
                str(e),
            )
...
"""
        ...
        return cloud_config.format(
            ...
            or DockerBenchmark.image_name(self.framework_def),
