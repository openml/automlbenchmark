...
):  # type: ignore  #  Split up to two functions, avoid "aliasing"
    """
...
def to_data_frame(
    obj: object, column_names: Iterable[str] | None = None
) -> pd.DataFrame:
    if obj is None:
        return pd.DataFrame()
    ...
    if isinstance(obj, dict):
        orient = cast(
            Literal["columns", "index"], "columns" if columns is None else "index"
        )
    ...
    raise ValueError(
        "Object should be a dictionary {col1:values, col2:values, ...} "
...
class Encoder(TransformerMixin):
    ...
    def __init__(
        ...
        self.missing_values = (
            set(missing_values).union([None]) if missing_values else {None}
        )
        ...
        if type == "label":
            ...
            raise ValueError(
                "Encoder `type` should be one of {}.".format(["label", "one-hot"])
            )
    ...
    def fit(self, vector: Iterable[str] | None) -> "Encoder":
        ...
        if vector is None:
            raise ValueError(
                "`vec` can only be `None` if Encoder was initialized with type 'label' or 'one-hot'."
            )
        ...
        self.classes = (
            ...
            else np.unique(np.insert(vec, 0, self.missing_replaced_by))
        )

        if self._mask_missing:
            self.missing_encoded_value = self.delegate.fit_transform(
                self._reshape(cast(np.ndarray, self.classes))
            )[0]
        else:
    ...
    def transform(self, vec: str | np.ndarray, **params: Any) -> str | np.ndarray:
        ...
        if self._mask_missing or self._encode_missing:
            ...
            if any(mask):
                ...
                res = self.delegate.transform(self._reshape(vector), **params).astype(
        ...
        if self.normalize_fn:
            vector = self.normalize_fn(vector)
        return return_value(
            self.delegate.transform(self._reshape(vector), **params).astype(
                self.encoded_type, copy=False
            )
        )
...
def impute_dataframe(
    ...
    *X_s: Iterable[pd.DataFrame],
    ...
    if isinstance(strategy, dict):
        ...
        imputed = _impute_pd(
            X_fit, *X_s, missing_values=missing_values, strategy=strategy
        )
...
def _restore_dtypes(
    X_np: np.ndarray, X_ori: pd.DataFrame | pd.Series | np.ndarray
) -> pd.DataFrame | pd.Series | np.ndarray:
    if isinstance(X_ori, pd.DataFrame):
        df = pd.DataFrame(
            X_np, columns=X_ori.columns, index=X_ori.index
        ).convert_dtypes()
