...
class Scoreboard:
    ...
    def save_df(data_frame, path, append=False):
        ...
        is_default_index = data_frame.index.name is None and not any(
    ...
    def __init__(
        ...
        Args:
            ...
            benchmark_name: Benchmark associated with the scores.
            task_name: Task identifier for the scores.
            scores_dir: Directory path for score files; a default is used if not specified.
            autoload: Determines if scores should be automatically loaded when not provided.
        """
    ...
    def as_data_frame(self):
        ...
        dynamic_cols = [
            col
            for col in df.columns
            if col not in index and col not in fixed_cols and col not in metrics_cols
        ]
        dynamic_cols.sort()
    ...
    def as_printable_data_frame(self, verbosity=3):
        ...
        Args:
            verbosity (int, optional): Level of detail for the output. The value determines
                ...
                - 0: No columns.
                - 1: Basic metadata columns (e.g., task, fold, framework, constraint, result,
                  metric, info).
        ...
        def as_int_or_str(val: Any) -> str | int:
            return (
                int(val)
                if isinstance(val, (float, int)) and not np.isnan(val)
                else none_like_as_empty(val)
            )
        ...
        high_precision_float_cols = [
            col
            for col in df.select_dtypes(include=[float]).columns
            if col not in ([] + nanable_int_cols + low_precision_float_cols)
        ]
        for col in force_str_cols:
            df[col] = df[col].map(none_like_as_empty)
        for col in nanable_int_cols:
            df[col] = df[col].map(as_int_or_str)
        for col in low_precision_float_cols:

            def format_low_precision_float(val: float) -> str:
                return ("{:.1g}" if val < 1 else "{:.1f}").format(val)

            # The .astype(float) is required to maintain NaN as 'NaN' instead of 'nan'
            df[col] = df[col].map(format_low_precision_float).astype(float)
        ...
        cols = (
            []
            if verbosity == 0
            else ["task", "fold", "framework", "constraint", "result", "metric", "info"]
            if verbosity == 1
            else [
                "id",
                "task",
    ...
    def append(self, board_or_df, no_duplicates=True):
        to_append = (
            board_or_df.as_data_frame()
            if isinstance(board_or_df, Scoreboard)
            else board_or_df
        )
    ...
    def path(self):
        ...
        if self.framework_name:
            if self.task_name:
                ...
                file_name = (
                    f"{self.framework_name}{sep}benchmark_{self.benchmark_name}.csv"
                )
            else:
...
class TaskResult:
    ...
    def load_predictions(predictions_file):
        ...
        if os.path.isfile(predictions_file):
            ...
            log.warning(
                "Predictions file `%s` is missing: framework either failed or could not produce any prediction.",
                predictions_file,
            )
    ...
    def load_metadata(metadata_file):
        ...
        if os.path.isfile(metadata_file):
            ...
            log.warning(
                "Metadata file `%s` is missing: framework either couldn't start or implementation doesn't save metadata.",
                metadata_file,
            )
    ...
    def save_predictions(
        dataset: Dataset,
        output_file: str,
        predictions: Union[A, DF, S] = None,
        truth: Union[A, DF, S] = None,
        probabilities: Union[A, DF] = None,
        probabilities_labels: Union[list, A] = None,
        ...
        Parameters:
            ...
            predictions: Predicted labels (array, DataFrame, or Series), which may be encoded.
            truth: Actual labels (array, DataFrame, or Series); if omitted, test labels from the dataset are used.
            probabilities: Optional probability scores (array or DataFrame) for each class.
            probabilities_labels: Optional list or array of labels for the probability columns; defaults to target classes.
            optional_columns: Additional columns to append to the output DataFrame.
            target_is_encoded: Flag indicating whether predictions and truth are already label encoded.
            preview: If True, logs a preview of the output DataFrame before saving.
        """
        ...
        if (
            ...
            and isinstance(truth, scipy.sparse.sparray)
            and truth.shape[1] == 1
        ):
        ...
        if probabilities is not None:
            prob_cols = (
                ...
                else dataset.target.label_encoder.classes
            )
    ...
    def score_from_predictions_file(cls, path):
        ...
        file_pat = (
            rf"(?P<framework>[\w\-]+?){sep}(?P<task>[\w\-]+){sep}(?P<fold>\d+)\.csv"
        )
        ...
        if benchmark:
            try:
                ...
                log.warning(
                    "Failed to fetch task from benchmark definition to load predictions: %s",
                    e,
                )
    ...
    def __init__(
        ...
        Args:
            ...
            constraint (str): A string specifying any constraints applicable to the task.
            predictions_dir (str, optional): The directory to store prediction results. If not provided, a default directory is computed based on the current configuration.
            metadata (Namespace, optional): Additional metadata for the task.
        """
        self.task = task_def
        self.fold = fold
        ...
        self.predictions_dir = (
            ...
            if predictions_dir
                ...
                rconfig().output_dir, rconfig().sid, ["predictions"]
            ).predictions
        )
    ...
    def compute_score(self, result=None, meta_result=None):
        ...
        if inference_times := Namespace.get(meta_result, "inference_times"):
            for data_type, measurements in Namespace.dict(inference_times).items():
                for n_samples, measured_times in Namespace.dict(measurements).items():
                    entry[f"infer_batch_size_{data_type}_{n_samples}"] = np.median(
                        measured_times
                    )
        ...
        def set_score(score):
            ...
            if (
                score.higher_is_better is False
            ):  # if unknown metric, and higher_is_better is None, then no change
        ...
        entry |= Namespace(
            {
                k: v
                for k, v in meta_result
                if k not in required_meta_res and k != "inference_times"
            }
        )
    ...
    def _predictions_file(self):
        return os.path.join(
            self.predictions_dir, self.task.name, str(self.fold), "predictions.csv"
        )

    @property
    def _metadata_file(self):
        return os.path.join(
            self.predictions_dir, self.task.name, str(self.fold), "metadata.json"
        )
    ...
    def evaluate(self, metric):
        ...
        if hasattr(self, metric):
            ...
            eval_res += Namespace(
                ...
                message=f"Unsupported metric `{metric}` for {pb_type} problems",
            )
        return eval_res
...
class NoResult(Result):
    ...
    def evaluate(self, metric):
        ...
        if metric is None:
            ...
            eval_res += Namespace(
                higher_is_better=None, message=f"Unsupported metric `{metric}`"
            )
        else:
            eval_res.higher_is_better = get_metadata(
                _supported_metrics_.get(metric), "higher_is_better"
            )
        return eval_res
...
class ErrorResult(NoResult):
    def __init__(self, error):
        msg = "{}: {}".format(
            type(error).__qualname__ if error is not None else "Error", error
        )
...
class ClassificationResult(Result):
    ...
    def __init__(self, predictions_df, info=None):
        ...
        self.target = Feature(
            0, "target", "category", values=self.classes, is_target=True
        )
        self.type = (
            DatasetType.binary if len(self.classes) == 2 else DatasetType.multiclass
        )
    ...
    def auc(self):
        ...
        if self.type == DatasetType.multiclass:
            raise ResultError(
                "For multiclass problems, use `auc_ovr` or `auc_ovo` metrics instead of `auc`."
            )
        else:
    ...
    def _autoencode(self, vec):
        needs_encoding = not _encode_predictions_and_truth_ or (
            isinstance(vec[0], str) and not vec[0].isdigit()
        )
    ...
    def _fbeta(self, beta):
        average = (
            ClassificationResult.multi_class_average
            if self.type == DatasetType.multiclass
            else "binary"
        )
        return float(
    ...
    def _per_class_errors(self):
        return [
            (s - d) / s for s, d in ((sum(r), r[i]) for i, r in enumerate(self._cm()))
        ]
...
class TimeSeriesResult(RegressionResult):
    def __init__(self, predictions_df, info=None):
        ...
        required_columns = {
            ...
            "predictions",
            "repeated_item_id",
            "repeated_abs_seasonal_error",
        }
        if required_columns - set(self.df.columns):
            raise ValueError(
                f"Missing columns for calculating time series metrics: {required_columns - set(self.df.columns)}."
            )

        quantile_columns = [
            column for column in self.df.columns if column.startswith("0.")
        ]
        unrecognized_columns = [
            column
            for column in self.df.columns
            if column not in required_columns and column not in quantile_columns
        ]
        if len(unrecognized_columns) > 0:
            raise ValueError(
                f"Predictions contain unrecognized columns: {unrecognized_columns}."
            )
        ...
        self.abs_seasonal_error = self.df["repeated_abs_seasonal_error"].values.astype(
            float
        )
        ...
        if (~np.isfinite(self.predictions)).any() or (
            ~np.isfinite(self.quantile_predictions)
        ).any():
        ...
        if len(set(unique_item_ids_counts)) != 1:
            raise ValueError(
                f"Error: Predicted sequences have different lengths {unique_item_ids_counts}."
            )
    ...
    def wape(self):
        ...
        return np.sum(np.abs(self.truth - self.predictions)) / np.sum(
            np.abs(self.truth)
        )
    ...
    def wql(self):
        ...
        """
        return self._quantile_loss_per_step().mean(axis=1).sum() / np.sum(
            np.abs(self.truth)
        )
    ...
    def sql(self):
        ...
        This makes scores comparable across different items.
        """
        pl_per_item = self._itemwise_mean(
            self._quantile_loss_per_step().mean(axis=1) / self.abs_seasonal_error
        )
