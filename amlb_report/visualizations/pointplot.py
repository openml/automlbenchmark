...
def draw_pointplot(
    ...
    with (
        sb.axes_style("whitegrid", rc={"grid.linestyle": "dotted"}),
        ...
        join_styles = dict(
            ...
            dashdotted_loose=dict(
                join=True, linestyles=[(0, (3, 2, 1, 2))] * len(hues)
            ),
            dashdotted_xloose=dict(
                join=True, linestyles=[(0, (3, 5, 1, 5))] * len(hues)
            ),
        )
        marker_styles = dict(
...
def draw_score_pointplot(
    ...
    **kwargs,
    ...
    sort_by = (
        x_sort_by
        if callable(x_sort_by)
        else None
        if not metadata or not isinstance(x_sort_by, str)
    ...
    df = (
        plot_df if type_filter == "all" else plot_df[plot_df.index.get_loc(type_filter)]
    )
