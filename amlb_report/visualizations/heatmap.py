...
def draw_heatmap(
    df, x_labels=True, y_labels=True, title=None, xlabel=None, ylabel=None, **kwargs
):
...
def draw_score_heatmap(
    ...
    **kwargs,
    ...
    df = results.groupby(["type", "task", "framework"])[col].mean().unstack()
    df = df if type_filter == "all" else df[df.index.get_loc(type_filter)]
    sort_by = (
        y_sort_by
        if callable(y_sort_by)
        else None
        if not metadata or not isinstance(y_sort_by, str)
