...
def add_imputed_mark(values, imp, val_type=float, val_format=None):
    ...
    return values.astype(object).combine(
        imp, lambda val, imp: format_value(val, " ({:.0g})".format(imp) if imp else "")
    )


def render_summary(
    col, results, show_imputations=True, filename=None, float_format=None
):
    ...
    if show_imputations and "imp_result" in results.columns:
        ...
        df = df.combine(
            imputed_df,
            ft.partial(
                add_imputed_mark, val_format=lambda *v: (float_format + "%s") % tuple(v)
            ),
        )
    display(df, float_format=float_format)
    if filename:
        df.to_csv(
            create_file("tables", config.results_group, filename),
...
def render_leaderboard(
    col, results, aggregate=False, show_imputations=True, filename=None
):
    res_group = results.groupby(["type", "task", "framework"])
    df = (
        res_group[col].mean().unstack()
        if aggregate
        else results.pivot_table(
            index=["type", "task", "fold"], columns="framework", values=col
        )
    )
    df = df.apply(rank, axis=1, result_type="broadcast").astype(object)
    if show_imputations and "imp_result" in results.columns:
        imputed_df = (
            res_group["result", "imp_result"]
            .apply(lambda df: sum(imputed(row) for _, row in df.iterrows()))
    ...
    if filename:
        df.to_csv(
            create_file("tables", config.results_group, filename), float_format="%.f"
        )
    return df
