...
def test_serialize_pandas_dataframes(tmpdir):
    ...
    df = pd.DataFrame(
        dict(first=[1, 2.2, pd.NA, 3, 4.4], second=["a", "b", "c", "a", "b"])
    )
...
def test_serialize_sparse_matrix_reload_as_dense(tmpdir):
    ...
    reloaded = deserialize_data(
        path, config=ns(sparse_matrix_deserialized_format="dense")
    )
...
def test_serialize_sparse_matrix_reload_as_array(tmpdir):
    ...
    reloaded = deserialize_data(
        path, config=ns(sparse_matrix_deserialized_format="array")
    )
    assert isinstance(reloaded, np.ndarray)
...
def test_serialize_sparse_numerical_dataframe_to_parquet(tmpdir):
    ...
    ser_config = ns(
        pandas_serializer="parquet", sparse_dataframe_deserialized_format=None
    )
    dfs = pd.DataFrame(
        dict(
            first=[0, 0, 0, 3.3],
...
def test_serialize_mixed_dataframe_to_parquet(tmpdir):
    ...
    dfm = pd.DataFrame(
        dict(
            ...
            second=pd.arrays.SparseArray(
                [4.4, 0, 0, 0], dtype=pd.SparseDtype(float, 0)
            ),
