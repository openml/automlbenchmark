...
def test_interruption_behaves_like_a_keyboard_interruption_by_default():
    timeout = 1
    with Timer() as t, pytest.raises(KeyboardInterrupt):
...
def test_interruption_with_sig_as_None():
    timeout = 1
    with Timer() as t, InterruptTimeout(timeout_secs=timeout, sig=None):
        for i in range(100):
            time.sleep(0.1)
    assert t.duration - timeout < 1
    assert i < 11
...
def test_interruption_with_sig_as_error_class():
    ...
    with (
        ...
        InterruptTimeout(timeout_secs=timeout, sig=TimeoutError),
    ):
        for i in range(100):
            time.sleep(0.1)
    assert t.duration - timeout < 1
...
def test_interruption_with_sig_as_error_instance():
    timeout = 1
    with Timer() as t, pytest.raises(TimeoutError, match=r"user provided error"):
...
def test_interruption_with_sig_as_signal():
    ...
    with signal_handler(signal.SIGTERM, _handler):
        timeout = 1
        with Timer() as t, pytest.raises(TimeoutError, match=r"from handler"):
