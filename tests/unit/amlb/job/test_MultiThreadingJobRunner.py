...
def test_run_multiple_jobs():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}", result=i, steps=seq_steps, verbose=True, duration_secs=0.1
        )
...
def test_run_multiple_jobs_with_delay():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}", result=i, steps=seq_steps, verbose=True, duration_secs=1
        )
        for i in range(n_jobs)
    ]
...
def test_stop_runner_during_job_run():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}", duration_secs=0.5, result=i, steps=seq_steps, verbose=True
        )
        for i in range(n_jobs)
    ]
    ...
    assert (
        len(list(filter(lambda s: s[1] == "completing", seq_steps)))
        + len(cancelled_jobs)
        == n_jobs
    )
...
def test_reschedule_job_default():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}", duration_secs=1, result=i, steps=seq_steps, verbose=True
        )
        for i in range(n_jobs)
    ]
    ...
    assert seq_steps.index(("job_4", "completing")) > seq_steps.index(
        ("job_5", "completing")
    )
...
def test_reschedule_job_enforce_job_priority():
    ...
    runner = MultiThreadingJobRunner(
        jobs,
        parallel_jobs=3,
        delay_secs=0.2,
...
def test_reschedule_job_high_parallelism():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}",
            duration_secs=random.randint(150, 250) / 10,
            result=i,
            steps=seq_steps,
            verbose=True,
        )
    ...
    for state in [
        ...
        assert len(
            list(filter(lambda s: s == state, normal_job_steps))
        ) == n_jobs - len(rescheduled_job_names)

    assert len(
        list(filter(lambda s: s == "created", rescheduled_job_steps))
    ) == 1 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "starting", rescheduled_job_steps))
    ) == 3 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "running", rescheduled_job_steps))
    ) == 3 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "rescheduling", rescheduled_job_steps))
    ) == 2 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "completing", rescheduled_job_steps))
    ) == 1 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "stopping", rescheduled_job_steps))
    ) == 1 * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "stopped", rescheduled_job_steps))
    ) == 1 * len(rescheduled_job_names)
...
def test_reschedule_does_not_lead_to_runner_cancellation_on_high_parallelism():
    ...
    jobs = [
        DummyJob(
            name=f"job_{i}",
            duration_secs=random.randint(10, 20) / 10,
            result=i,
            steps=seq_steps,
            verbose=True,
        )
    ...
    for job in jobs:
        job._on_state = ft.partialmethod(_on_state, ori=job._on_state).__get__(
            job, None
        )
    ...
    assert len(
        list(filter(lambda s: s == "starting", rescheduled_job_steps))
    ) == attempts * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "running", rescheduled_job_steps))
    ) == attempts * len(rescheduled_job_names)
    assert len(list(filter(lambda s: s == "rescheduling", rescheduled_job_steps))) == (
        attempts - 1
    ) * len(rescheduled_job_names)
    assert len(
        list(filter(lambda s: s == "completing", rescheduled_job_steps))
    ) == 1 * len(rescheduled_job_names)
    assert len(
