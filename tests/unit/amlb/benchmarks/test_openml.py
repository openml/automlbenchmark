...
from amlb.benchmarks.openml import is_openml_benchmark, load_openml_task, load_oml_benchmark
...
def test_load_openml_task(mocker, oml_task, oml_dataset):
    ...
    mocker.patch("openml.datasets.get_dataset", new=mocker.Mock(return_value=oml_dataset))
...
def test_load_oml_benchmark_from_task(mocker, oml_task, oml_dataset, domain):
    ...
    mocker.patch("openml.config")  # Required to avoid load_oml_benchmark affecting global state
    mocker.patch("openml.tasks.get_task", new=mocker.Mock(return_value=oml_task))
    mocker.patch("openml.datasets.get_dataset", new=mocker.Mock(return_value=oml_dataset))
...
def test_load_oml_benchmark_from_suite(mocker, domain):
    ...
    mock_datasets = pd.DataFrame([[21, "foo", "bar"]], columns=["did", "name", "description"])
    benchmark_name = f"{domain}/s/{mock_suite.id}"

    mocker.patch("openml.config")  # required to avoid load_oml_benchmark affecting global state
    mocker.patch("openml.study.get_suite", new=mocker.Mock(return_value=mock_suite))
    mocker.patch("openml.datasets.list_datasets", new=mocker.Mock(return_value=mock_datasets))
